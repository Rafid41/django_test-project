...
def login_view(request):
    ...
    return render(request, "commons/login.html", context={"form": form})
...
def create_post(req):
    if req.user.is_authenticated:
        ...
        if req.method == "POST":
            ...
            print(user_id, category, title, img, description, vacancy)
...
def home(req, category=""):
    if req.user.is_authenticated:
        ...
        return render(
            req,
            "home.html",
            {"posts": Post.objects.all(), "titles": Post.objects.all()},
        )
    else:
        return redirect("login")
    ...
    if req.user.is_authenticated:
        if category != "":
            return render(
                ...
                "home.html",
                {
                    "posts": Post.objects.filter(title=category),
                    "titles": Post.objects.all(),
                },
            )
        return render(
...
def delete_post_page(req):
    if req.user.is_authenticated:
        return render(
            req,
            "delete_post_page.html",
            {"posts": Post.objects.filter(user=req.user.id)},
        )
    else:
        return redirect("login")
    ...
    if req.user.is_authenticated:
        if id is not None:
            ...
            if not OrderList.objects.filter(user=req.user.id, product_id=id):
                OrderList(user=req.user.id, product_id=id).save()
        ...
        return render(
            ...
            {"contents": Post.objects.filter(id__in=product_list)},
        )
    else:
        return redirect("login")
...
def handle_orders(req):
    if req.user.is_authenticated:
        if req.user.is_superuser:
            ...
            return render(
                req,
                "orders/handle_orders.html",
                {"contents": PlacedOrder.objects.all()},
            )
        else:
